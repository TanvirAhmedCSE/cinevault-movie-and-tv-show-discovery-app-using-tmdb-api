...
import 'movie_all_genres_details_screen.dart';
import 'movie_genre_details_screen.dart';
...
class _GenresScreenState extends State<GenresScreen> {
  ...
  List<GenreModel> _filteredGenres = [];
  ...
  Future<void> _fetchGenres() async {
    try {
      final res = await http.get(
        Uri.parse(
          'https://api.themoviedb.org/3/genre/movie/list?api_key=$_kApiKey&language=en-US',
        ),
      );
  ...
  Future<void> _searchMovies(String query) async {
    ...
    try {
      final res = await http.get(
        Uri.parse(
          'https://api.themoviedb.org/3/search/movie?api_key=$_kApiKey&language=en-US&query=${Uri.encodeComponent(query)}&page=1',
        ),
      );
      if (res.statusCode == 200) {
        final data = jsonDecode(res.body);
            ...
            Expanded(
              child: TextField(
                ...
                decoration: InputDecoration(
                  hintText: 'Search genres or movies...',
                  hintStyle: TextStyle(
  ...
  Widget _buildGenresGrid() {
    ...
    // Build items: "All" first, then sorted genres
    final allItem = _GenreGridItem(
      label: 'All',
      ...
      onTap: () => Navigator.push(
        context,
        _fadeRoute(AllGenresDetailsScreen(allGenres: _allGenres)),
      ),
    );
    ...
    return GridView.builder(
      ...
      itemCount: _filteredGenres.length + 1,
      itemBuilder: (context, index) {
        if (index == 0) return allItem;
        final genre = _filteredGenres[index - 1];
        return _GenreGridItem(
          label: genre.name,
          icon: _genreIcon(genre.name),
          isAll: false,
          index: index,
          onTap: () => Navigator.push(
            context,
            _fadeRoute(GenreDetailsScreen(genre: genre, allGenres: _allGenres)),
          ),
        );
      },
    );
        ...
        children: [
              ...
              itemBuilder: (context, index) {
                ...
                return _GenreGridItem(
                  ...
                  onTap: () => Navigator.push(
                    context,
                    _fadeRoute(
                      GenreDetailsScreen(genre: genre, allGenres: _allGenres),
                    ),
                  ),
                );
              },
          ...
          if (_isSearching)
            ...
            GridView.builder(
              ...
              itemBuilder: (context, index) =>
                  _MovieGridCard(movie: _searchedMovies[index]),
            ),
        ],
      ),
    );
...
class _GenreGridItem extends StatelessWidget {
  ...
  // Subtle color palette for variety
  Color _cardColor() {
    if (isAll) return const Color(0xFFE50914);
    final colors = [
